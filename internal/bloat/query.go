...
SELECT
    ...
    (CASE WHEN relpages > est_pages_ff
        THEN (bs::bigint * (relpages - est_pages_ff))
        ELSE 0
...
func IndexBloatQuery() string {
	return indexBloatQuery
}
...
func FetchIndexBloat(ctx context.Context, db *sql.DB) ([]IndexBloat, error) {
	rows, err := db.QueryContext(ctx, indexBloatQuery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var row IndexBloat
		if err := rows.Scan(&row.Schema, &row.Table, &row.Index, &row.RealSize, &row.BloatSize, &row.BloatPct); err != nil {
			return nil, err
		}
